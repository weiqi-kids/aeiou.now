...
⚠ **訂正(2026-09-19)**:上面那句「名次也真的退了:twn 9.5 → 65.8」是**組成變化造成的假象**,
不要引用。國別平均名次的「後」值是由**另一批查詢**構成的 —— 原本排前面的查詢整批不再出現,
剩下的都是名次 60~90 的長尾,平均自然變差。
同一組 query×page 前後比(`node scripts/freeze-exit-check.mjs` 的第 ③ 條)是 **10.4 → 10.2,沒有退**。
🔴 但那一條有**存活者偏誤**:斷崖前有量的 46 組裡只有 16 組(34.8%)斷崖後還有曝光,
而名次只在有曝光時才被記錄 —— 掉到第 90 名的查詢是「退出樣本」,不是「把平均拉高」。
所以正確的說法只有一句:**大多數查詢上站台根本沒被端出來,而沒被端出來就量不到名次。**
Google 這一側問不出更多了。
...
🔴 **復原判準已改寫成可執行的腳本**(2026-09-19):`node scripts/freeze-exit-check.mjs`。
舊判準「舊 Topic 主頁每日曝光回到約 150–200」作廢 —— 它掛在被遮罩的 `site_search_daily` 上,
而擴張前的實測基線(`['date']` 真值尺度)是 **08-16~08-25 每日 144.1**(10 天 1,441 曝光 5 點擊)。
照舊判準執行,12-16 必然讀出「沒回來」、凍結自動延長吃掉整個 Q4。

新判準三條並列,腳本直接印判定與分支:
① 站級 `dim='date'` 的 7 日移動平均 vs 基線的 70%;
② 固定 cohort(08-26 逐國頁上線前就有觀測的 152 個路徑,不含逐國頁與假日總表),Q4 節日另列一欄
   防止季節性反彈被讀成解除;
③ 固定 query×page cohort 的曝光加權名次,**且必須通過存活率 >= 50% 才算可判定**。

2026-09-19 跑出來的現況:① 26.3/日(基線的 18.2%)❌　② 非 Q4 cohort 4.2/日(3.0%)❌
③ 存活率 34.8% → **不可判定**。
判定是:**Google 這一側已經問不出更多了** —— 既不能證明降權還在,也不能證明只是曝光資格問題。
對應的下一步不是「再等一個月」,是換一個 Google 以外的判官(見下)。

### 下一步:換一個判官,不要繼續等

現行計劃押在「12-16 Google 會放人」,但上面的判定說 Google 這一側量不出判別力。
目前沒有任何獨立證據能分辨這兩件事:

- (a) 內容有需求,是 Google 單方面壓著 → 等是對的
- (b) 內容本來就沒人要,Google 只是先反映出來 → 等是浪費整個 Q4
...
4. **凍結出場檢查腳本**(`scripts/freeze-exit-check.mjs`):三條判準＋季節性對照＋存活率守門,
   唯讀。取代 09-17 那句做不到的散文判準。
5. **公告溯源事件流**(`holiday_announcements` 表):`announcement-watch.mjs` 每天看七國政府公告頁,
   以前判斷完就把觀察丟掉。現在**狀態改變時**追記一列(first_seen/appeared/matched/changed/
   disappeared/overdue/transcribed),append-only。理由同 GSC:沒有人保存政府公告頁的歷史快照,
   今天不存以後補不回來。第一輪就記到 ID 2028 的 SKB 公告出現、US 2027/2028 內容變動。
   🔴 它仍然不改任何日期 —— 記的是「什麼時候在哪裡看到什麼」。
6. **在地來源的內容層加上容忍**(`update-local-data.mjs` 的 `CONTENT_TOLERANCE`,預設 3):
   原本內容層第一次對不上就隔離、第一次對上就解除,於是 binus.ac.id 每小時翻一次 ——
   `data/places/jakarta.json` 三天被改寫 11 次,印尼站讀者看著一個地點出現又消失,
   它掛的 Topic 頁每輪對 Google 宣告改版。傳輸層早就有 3 輪容忍,內容層補齊。
7. **url-inspection `--report` 補逐 host 覆蓋度與本輪進度**:一輪跨三到四天,中途看到的一定是半輪。
   09-18 我自己把「hi/id/cn/br 是 0 筆」誤讀成那幾站有問題,實際上只是還沒排到
   (實測 1,136 筆/次,受 50 分鐘軟上限節流)。現在報表會明說。
