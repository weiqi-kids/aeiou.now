...
- [ ] 🔴 **真正的瓶頸:摘要答錯國家**(2026-08-25 查出,接在上一項後面)。
      ...
      也就是說 2026-08-21 拍板的「description 第一句是**本市場那一國**」這條規則,
      正好答錯了每一個排得上去的查詢。⚠ **這不是推翻 08-21**:
      「把日期換成制度」那半是對的(純日期型名次 32.7→18.2);要換的是「哪一國」的選法。
      判準已做進工具:`seo-health.mjs` 的「摘要答對國家了嗎」那一層會直接列出錯配清單。
      **修法屬產品文案 + 用戶拍板過的決策,動手前要用戶點頭**(見下方「要用戶決定」)。
      候選:description 第一句改講**該 Topic 搜尋需求集中的那一國**,而不是本市場那一國;
      需求集中在哪一國有資料可判(`topic_search_metrics` 有 country 維度)。
