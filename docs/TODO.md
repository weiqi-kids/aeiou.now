...
- [x] **十月中補收提醒已排**(2026-08-30):`/etc/cron.d/aeiou-event-refill`,
      10 月的 13/20/27 日 05:23 各記一筆 `event-refill-reminder` 到 jobs 表
      (`partial_success`,不是失敗 —— 標 failed 會進重試與 dlq)。
      為什麼要另外排而不靠 `local-event-runway`:那支只說得出「現在剩幾場」,
      說不出「現在該回去收了」—— **存量見底與公告出爐是兩件事**,前者可能在
      公告出來之前就先報了,那時候去收一樣收不到。
      排三次是因為公告陸續出來,一次抓不完。
      🔴 **收完之後請刪掉那個檔**:`rm /etc/cron.d/aeiou-event-refill`。
      它是獨立檔,不在 `/etc/cron.d/aeiou` 核心管線裡。

- [🅤] **shopping-festivals 的 BR 內文有兩處事實出處已下架,用戶決定保留**(2026-08-30):
      `procon.df.gov.br` 那兩個來源 404 之後換成了活的頁面,但新來源撐不起內文的
      「局長 Marcelo Nascimento 的姓名與其建議」與「2025 年提醒:促銷拉長到整個
      十一月、跨出零售業」這兩段。**用戶明示不刪**,所以這是有意識的保留,
      不是漏掉的 —— 下一個 session 不要「順手修正」把它刪了。
      要補救的正解是找到能支撐這兩項的活官方頁面,不是刪內文。
      ⚠ 2026-09-17 01:48 的 commit 766213fa 仍把七語 BR 段落改寫、拿掉了這兩項(換來源時「順手修正」了)。
      同日已還原,並把兩個 procon.df 網址以 `retired=2026-09-17` 掛回 BR observance
      (不印、不驗、仍記為出處;活來源是 senacon + planalto)。這正是 retired= 語法的設計用途
      (`docs/03-topic-content.md` 規則 2)。
