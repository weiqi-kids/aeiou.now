...
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/; // YYYY-MM-DD(UTC 當日),每日世界一問 participation 查詢用
...
async function questionResults(env, questionId, anonId) {
  const rows = (
    await env.DB.prepare(
...
async function handleQuestionResults(request, env, questionId, cors) {
...
async function handleVote(request, env, ctx, cors) {
...
async function handleParticipation(env, url, cors) {
...
async function handleSyncQuestions(request, env) {
  const body = await readJson(request);
