...
### 定版前不要七語系、不要部署(2026-08-11 用戶指示)

版面還在改的期間,**只做 `zh-TW`**。改完就 commit 並**推 zh-TW 上線**(用戶是看線上網址在確認的,
只給截圖等於讓他一直看到舊版);七語翻譯與七站部署等定版後一次做。

| 定版前 | 定版後(用戶說「就這樣」才做) |
|---|---|
| 只 build `zh-TW` | 七語系 build |
| 新 i18n key 只寫 `zh-TW`,其餘六語填佔位 | 一次統一翻譯 pass |
| 只手動推 zh-TW 一站 | 七站部署 + 驗指紋 |

> **理由(用戶原話)**:「還沒有定版,你不用發佈到其他語系、也不用翻譯啊！浪費 token 嗎？」
> 每輪七語 build ×7、部署 ×7、等 Pages 傳播、翻譯下一輪就砍掉的字串,全是白做。
