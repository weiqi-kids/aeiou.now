...
// 「這些其實都是 Topic 的排序」。
//
// 所以「附近」不是一份店家清單、「活動」也不是一份活動清單——它們是 Topic 的排序:
//   · 附近訊息 = 全部 Topic,有在地資訊(places/<city>.json 有條目)的排前面
//   · 活動資訊 = 全部 Topic,有活動(events/<city>.json 有條目)的排前面
// 沒有資料的 Topic 仍在靜態 HTML 裡,只是排在後段,由頁面 JS 先顯示前 10 筆再逐批展開。
